...
import { encodeGistSnapshot } from "../src/shared/notesEnvelope";
...
export async function mockGitHub(page: Page, snapshot = sampleSnapshot()) {
  const content = await encodeGistSnapshot(snapshot, "firstlight-e2e-token");
  await page.route("https://api.github.com/**", async (route) => {
    expect(route.request().method(), "Online must never write to GitHub").toBe("GET");
