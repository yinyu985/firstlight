...
- 上传的 `firstlight.json` 使用两空格缩进和换行，方便直接阅读书签、设置和笔记元数据；大文件仍可能触发 GitHub 预览截断，完整内容可通过 Gist 的原始文件入口查看。
- Notes、显示设置和完整书签栏共同进入 `firstlight.json`。仅 Notes 正文使用原生 gzip + HKDF-SHA-256 / AES-256-GCM；标题、ID、时间在 `notes.items` 中可直接检查，书签和设置保持明文。跨设备及 Online 使用完全相同的 Gist token，无第二个密码。升级前的明文 Notes 不会自动同步；更新所有客户端后，可在扩展中用一次显式 `UPLOAD` 将经过校验的旧快照替换为当前加密格式。
